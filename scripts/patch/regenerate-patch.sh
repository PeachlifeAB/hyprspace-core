...
usage() {
    cat <<'EOF'
Usage: ./scripts/patch/regenerate-patch.sh <patch-name> [--add-file <path> ...]
...
require_live_tree() {
    [[ -d "$LIVE_TREE/.git" ]] || die "Live AeroSpace checkout not found at $LIVE_TREE. Run ./scripts/patch/refresh-workspace.sh first."
}
