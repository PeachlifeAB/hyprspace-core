...
ensure_sketchybar_companion_assets() {
    ...
    shopt -s nullglob
    local plugins=("$ROOT_DIR/artifacts/configs/sketchybar/plugins/"*.sh)
    local helpers=("$ROOT_DIR/artifacts/configs/sketchybar/helpers/"*)
    shopt -u nullglob
...
if [ -f "$SKETCHYBAR_CONFIG_FILE" ]; then
    ...
    ensure_sketchybar_companion_assets
    cp "$ROOT_DIR/artifacts/configs/sketchybar/sketchybarrc" "$SKETCHYBAR_CONFIG_FILE"
fi
