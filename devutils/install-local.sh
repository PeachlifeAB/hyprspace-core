...
for support_dir in devutils docs configs gfx AeroSpace; do
    if [ -e "$INSTALL_ROOT/$support_dir" ]; then
        rm -rf "$INSTALL_ROOT/$support_dir"
    fi
done
cp -R "AeroSpace/.release/devutils" "$INSTALL_ROOT/devutils"
cp -R "AeroSpace/.release/docs" "$INSTALL_ROOT/docs"
cp -R "AeroSpace/.release/configs" "$INSTALL_ROOT/configs"
cp -R "AeroSpace/.release/gfx" "$INSTALL_ROOT/gfx"
...
chmod +x "$INSTALL_ROOT/devutils/setup-dependencies.sh" "$INSTALL_ROOT/devutils/setup-hyprspace-config.sh" "$INSTALL_ROOT/devutils/setup-sketchybar-config.sh" "$INSTALL_ROOT/devutils/setup-macos-defaults.sh" "$INSTALL_ROOT/devutils/setup-wallpaper.sh"
