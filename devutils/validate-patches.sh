...
--help | -h)
    cat <<'EOF'
Usage: bash devutils/validate-patches.sh [--help]
...
Examples:
  bash devutils/validate-patches.sh
  PATCH_BIN=/opt/homebrew/bin/gpatch bash devutils/validate-patches.sh
EOF
    exit 0
    ;;
--lint-only)
...
lint_patch_format() {
    ...
    if [[ -n "$bad_markers" ]]; then
        ...
        echo "$bad_markers" >&2
        echo "   Run: bash devutils/normalize-series-patches.sh" >&2
        exit 1
    fi
