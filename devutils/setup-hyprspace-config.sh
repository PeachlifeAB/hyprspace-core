...
if [ ${#existing_configs[@]} -gt 0 ]; then
    ...
    mkdir -p "$CONFIG_DIR"
    render_starter_config "$ROOT_DIR/configs/hyprspace-config.toml" "$PREFERRED_CONFIG_DEST"
fi
...
cp "$ROOT_DIR/configs/docs/README.md" "$CONFIG_README_DEST"
cp "$ROOT_DIR/configs/docs/ACKNOWLEDGMENTS.md" "$ACKNOWLEDGMENTS_DEST"
