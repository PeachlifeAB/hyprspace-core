...
require_opt_in RUN_INTEGRATION_TESTS "test-hyprspace-new-window-window-count.sh is in the integration tier."
log_dir="$root_dir/log/tests"
timestamp="$(date -u +%Y%m%dT%H%M%SZ)"
log_file="$log_dir/${timestamp}-hyprspace-new-window-window-count.log"
...
echo "[info] before_count=$before_count"

echo "[step] running new-window-or-open for $target_cli_app"
hyprspace_assert_server_running "before new-window-or-open"
...
echo "[info] after_count=$after_count"
test "$after_count" -gt "$before_count"

echo "[ok] running-app path created a new window"
