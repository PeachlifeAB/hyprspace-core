...
test -x "$install_prefix/libexec/hyprspace-init/hyprspace-notify-menubar"
test -x "$install_prefix/libexec/hyprspace-init/hyprspace-set-wallpaper"
...
rm -f "$launch_marker"
init_output="$(PATH="$fake_bin_dir:$PATH" OPEN_LOG="$open_log" LAUNCH_MARKER="$launch_marker" FAKE_OPEN_MODE=launch HYPRSPACE_HOME_OVERRIDE="$init_home" HYPRSPACE_SKIP_DEPENDENCY_SETUP=1 HYPRSPACE_SKIP_MACOS_DEFAULTS=1 HYPRSPACE_SKIP_SKETCHYBAR_SERVICE=1 HYPRSPACE_SKIP_WALLPAPER_SETUP=1 HYPRSPACE_INIT_ASSUME_DEFAULTS=1 script -q /dev/null "$install_prefix/bin/hyprspace" init 2>&1)"
printf '%s\n' "$init_output"
