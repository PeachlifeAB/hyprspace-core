...
log_file="$log_dir/${timestamp}-hyprspace-init-tty-runtime.log"
...
echo "[step] running packaged hyprspace init in a tty-backed runtime test mode via symlinked entrypoint"
if ! output="$(HYPRSPACE_INIT_TEST_MODE=runtime script -q /dev/null bash "$fake_install_root/bin/hyprspace" init 2>&1)"; then
    echo "[error] packaged hyprspace init tty runtime launch failed"
    printf '%s\n' "$output"
    exit 1
fi
...
echo "[ok] hyprspace init tty runtime bridge test passed"
